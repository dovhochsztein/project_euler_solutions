...
% set inputs={a,b,c}. These can be of any data type.
%
% dinputs (dimensional inputs): Should be a cell array of all inputs that
% intended to be looped through. These should generally be constructed as
% n dimensional matrices of sizes that correspond to ranges and can be done
% using NDGRID. If there are none, set dinputs={}. These can be matrices
% themselves which is accomplished by setting the corresponding dinput to a
...
% correponding dinput as a 5x5 cell array of 
%
...
if ~exist('dinputs','var') | ~exist('inputs','var')
    input_list={};
    for i=1:M
        input_list=union(input_list,handle_inputs(lpcmnds{i,3}));
    end
    if ~exist('inputs','var')
        inputs=cell(2,0);
    end
...
%check lpcmnds:
% if ~all(floor(outputs)==outputs)
%     error('lpcmnd error: invalid output number')
% end

%check lpcmnd order, replace text with numbers:
for i=1:M
    if isempty(lpcmnds{i,2})
        lpcmnds{i,2}=handle_inputs(lpcmnds{i,3});
    end
    for j=1:length(lpcmnds{i,2})
        
        if iscell(lpcmnds{i,2}{j})
            if length(lpcmnds{i,2}{j})==2
                if strcmp(lpcmnds{i,2}{j}{1},'input')
                    lpcmnds{i,2}{j}{1}=0;
                elseif strcmp(lpcmnds{i,2}{j}{1},'dinput')
                    lpcmnds{i,2}{j}{1}=1;
                elseif strcmp(lpcmnds{i,2}{j}{1},'output')
                    lpcmnds{i,2}{j}{1}=2;
                end
            else
        ...
        elseif ischar(lpcmnds{i,2}{j})
            if any(strcmp(lpcmnds{i,2}{j},inputs(1,:)))
                lpcmnds{i,2}{j}={0,find(strcmp(lpcmnds{i,2}{j},inputs(1,:)))};
            elseif any(strcmp(lpcmnds{i,2}{j},dinputs(1,:)))
                lpcmnds{i,2}{j}={1,find(strcmp(lpcmnds{i,2}{j},dinputs(1,:)))};
            elseif any(strcmp(lpcmnds{i,2}{j},outputs(1,:)))
                lpcmnds{i,2}{j}={2,find(strcmp(lpcmnds{i,2}{j},outputs(1,:)))};
            else
        ...
        if (lpcmnds{i,2}{j}{1}==2) && ~any(lpcmnds{i,2}{j}{2}==output_num(1:M-1))
...
for j=1:M
    for k=1:length(lpcmnds{j,2})
        if lpcmnds{j,2}{k}{1}==0
            try
                inputs{lpcmnds{j,2}{k}{2}};
            catch
                error(horzcat('lpcmnd error:  indexing issue on loopcommand ',num2str(j),', entry ',num2str(k)));
            end
        elseif lpcmnds{j,2}{k}{1}==1
            try
                dinputs{lpcmnds{j,2}{k}{2}};
            catch
                error(horzcat('lpcmnd error:  indexing issue on loopcommand ',num2str(j),', entry ',num2str(k)));
            end
        elseif lpcmnds{j,2}{k}{1}==2
            try
                varargout{lpcmnds{j,2}{k}{2}};
            catch
                error(horzcat('lpcmnd error:  indexing issue on loopcommand ',num2str(j),', entry ',num2str(k)));
            end
        else
...
for I=1:length(indices)
    ...
    for j=1:M
        ...
        for k=1:length(lpcmnds{j,2})
            if lpcmnds{j,2}{k}{1}==0
                inps{k}=inputs{2,lpcmnds{j,2}{k}{2}};
            elseif lpcmnds{j,2}{k}{1}==1
                if iscell(dinputs{2,lpcmnds{j,2}{k}{2}})
                    shape=size(dinputs{2,lpcmnds{j,2}{k}{2}});
                    inps{k}=zeros(shape);
                    for L=1:prod(shape)
                        inps{k}(L)=dinputs{2,lpcmnds{j,2}{k}{2}}{L}(index{:});
                    end
                else
                    inps{k}=dinputs{2,lpcmnds{j,2}{k}{2}}(index{:});
                end
            elseif lpcmnds{j,2}{k}{1}==2
                inps{k}=varargout{lpcmnds{j,2}{k}{2}}(index{:});
            end
        end
        varargout{output_num(j)}(index{:})=lpcmnds{j,3}(inps{:});
    end
end
