A={'8C','TS','KC','9H','4S','7D','2S','5D','3S','AC';
...
for i=1:1000
    ...
    for p=1:2
        ...
        q=1;
        multiplicities=[];
    while q<=length(rank{p})
        num=sum(rank{p}==rank{p}(q));
        multiplicities=[multiplicities,num];
        q=q+num;
    end
        
        
        is_flush(p)= (suits{p}{1}==suits{p}{2} && suits{p}{1}==suits{p}{3} && suits{p}{1}==suits{p}{4} && suits{p}{1}==suits{p}{5}); 
        is_straight(p)=all((rank{p}(2:end)-rank{p}(1:end-1))==1);
        
        if is_flush(p) && is_straight(p)
            level(p)==1;
            comp{p}=fliplr(rank{p});
        elseif max(multiplicities)==4
            level(p)==2;
            if multiplicities(1)==4
                comp{p}=[rank{p}(1),rank{p}(5)];
            else
                comp{p}=[rank{p}(5),rank{p}(1)];
            end
        elseif any(multiplicities==2) && any(multiplicities==3)
            level(p)=3;
        ...
        elseif is_flush(p)
            level(p)=4;
            comp{p}=fliplr(rank{p});
        elseif is_straight(p)
            level(p)=5;
            comp{p}=fliplr(rank{p});
        elseif any(multiplicities==3)
            level(p)=6;
            if multiplicities(1)==3
                comp{p}=[rank{p}(1),rank{p}(5),rank{p}(4)];
            elseif multiplicities(2)==3
                comp{p}=[rank{p}(4),rank{p}(5),rank{p}(1)];
            else
                comp{p}=[rank{p}(5),rank{p}(2),rank{p}(1)];
            end
        elseif sum(multiplicities==2)==2
            level(p)=7;
            if multiplicities(1)==1
                comp{p}=[rank{p}(5),rank{p}(3),rank{p}(1)];
            elseif multiplicities(2)==1
                comp{p}=[rank{p}(5),rank{p}(1),rank{p}(3)];
            else
                comp{p}=[rank{p}(3),rank{p}(1),rank{p}(5)];
            end
        elseif any(multiplicities==2)
            level(p)=8;
            if multiplicities(1)==2
                comp{p}=[rank{p}(1),rank{p}(5),rank{p}(3),rank{p}(4)];
            elseif multiplicities(2)==2
                comp{p}=[rank{p}(2),rank{p}(5),rank{p}(4),rank{p}(1)];
                elseif multiplicities(3)==2
                comp{p}=[rank{p}(3),rank{p}(5),rank{p}(2),rank{p}(1)];
            else
                comp{p}=[rank{p}(5),rank{p}(4),rank{p}(3),rank{p}(1)];
            end
        else
            level(p)=9;
            comp{p}=fliplr(rank{p});
    ...
    elseif level(1)==level(2);
        for m=1:length(comp{1})
            if comp{1}(m)>comp{2}(m)
                count=count+1;
                break
            elseif comp{1}(m)<comp{2}(m)
                break
            end
        end
    end
